...
export default function FundCard({ fund, index }: Props) {
  ...
  return (
    <motion.div
      ...
      whileHover={
        !open ? { y: -4, boxShadow: "0 16px 40px rgba(0,0,0,0.1)" } : {}
      }
    ...
    >
      ...
      <div className="p-5 flex flex-col gap-2.5 flex-1">
        <div className="flex items-center justify-between">
          <span
            className={`text-[11px] font-bold tracking-[0.05em] px-3 py-1 rounded-full ${s.badgeBg} ${s.badgeText}`}
          >
            {s.badgeLabel}
          </span>
        ...
        <h3 className="font-serif text-[21px] text-navy leading-snug">
          {fund.name}
        </h3>
        ...
        <div className="flex items-center gap-1.5">
          {Array.from({ length: 7 }, (_, i) => (
            <div
              key={i}
              className={`w-2.5 h-2.5 rounded-full shrink-0 ${i < fund.r ? s.riskDot : "bg-slate-200"}`}
            />
          ))}
          <span
            className={`text-[12px] font-semibold ml-1.5 ${RISK_COLOR[fund.r]}`}
          >
        ...
        {open && (
          ...
          >
            <div className="border-t border-slate-100 p-5 flex flex-col gap-4">
              ...
              <div>
                <div className="text-[10px] font-bold tracking-[0.07em] uppercase text-slate-400 mb-1.5">
                  Qué hace
                </div>
              ...
              <div>
                <div className="text-[10px] font-bold tracking-[0.07em] uppercase text-slate-400 mb-2">
                  Por qué está en la cartera
                </div>
                {fund.pros.map((pro) => (
                  <div key={pro} className="flex items-start gap-2.5 mt-1.5">
                    <div
                      className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${s.riskDot}`}
                    />
